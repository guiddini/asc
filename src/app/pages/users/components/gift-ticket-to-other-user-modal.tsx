...
const schema = Yup.object().shape({
  ...
  gifter_email: Yup.string()
    .email()
    .required("Vous devez sélectionner un email"),
});
...
interface GiftTicketToOtherUserModalProps extends assingTicketProps {
  gifter_email: string;
}

const GiftTicketToOtherUserModal = ({
  isOpen,
  setIsOpen,
  user,
  ...
  const { mutate, isLoading } = useMutation({
    mutationFn: async ({
      ...
      return await adminGiftTicketToOtherUserApi({
        user_id,
        user_has_ticket_id,
        gifter_email,
      });
  ...
  const submit = async (data: GiftTicketToOtherUserModalProps) => {
    console.log("Guft to other user : ", data);
    ...
    >
      <div className="modal-content">
        <div className="modal-header">
          <h2 className="fw-bolder">Gift ticket to other user</h2>
...
export default GiftTicketToOtherUserModal;
