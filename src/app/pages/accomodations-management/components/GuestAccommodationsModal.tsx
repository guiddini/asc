...
import { getGuestAccommodations } from "../../../apis/accommodations";
import moment from "moment";
import getMediaUrl from "../../../helpers/getMediaUrl";

export const GuestAccommodationsModal: React.FC<{
  isOpen: boolean;
  onClose: () => void;
  userId: string;
}> = ({ isOpen, onClose, userId }) => {
  const { data, isLoading } = useQuery({
    queryKey: ["guest-accommodations", userId],
    queryFn: () => getGuestAccommodations(userId),
  ...
  return (
    <Modal show={isOpen} onHide={onClose} centered size="lg" backdrop="static">
      <Modal.Header closeButton>
        <Modal.Title>Guest Accommodations</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        {isLoading ? (
