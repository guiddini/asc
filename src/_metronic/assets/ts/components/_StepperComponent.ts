...
class StepperComponent {
  ...
  private _setCurrentIndex = (index: number) => {
    // Skip if the provided index is out of bounds
    if (index < 1 || index > this.totalStepsNumber) {
      return;
    }
  ...
  private _updateTotalStepsNumber = (e: number) => {
    this.totalStepsNumber = e;
  };
  ...
  public initTotalSteps = (stepNumber: number) => {
    return this._updateTotalStepsNumber(stepNumber);
  };
